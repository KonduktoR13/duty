...
const NAME='my-shifts', VERSION=3
function db(): Promise<IDBDatabase> { return new Promise((resolve,reject)=>{ const r=indexedDB.open(NAME,VERSION); r.onupgradeneeded=()=>{ const d=r.result; if(!d.objectStoreNames.contains('months'))d.createObjectStore('months',{keyPath:'id'}); if(!d.objectStoreNames.contains('settings'))d.createObjectStore('settings'); if(!d.objectStoreNames.contains('pdfs'))d.createObjectStore('pdfs'); if(!d.objectStoreNames.contains('syncs'))d.createObjectStore('syncs',{keyPath:'id'}) }; r.onsuccess=()=>resolve(r.result); r.onerror=()=>reject(r.error) }) }
...
export const storage = {
  ...
  clear: async () => { const d = await db(); await Promise.all(['months', 'settings', 'pdfs', 'syncs'].map(store => new Promise<void>((res, rej) => { const t = d.transaction(store, 'readwrite'); t.objectStore(store).clear(); t.oncomplete = () => res(); t.onerror = () => rej(t.error) }))) }
}
